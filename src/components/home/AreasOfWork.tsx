        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {areas.map((area, index) => (
            <Reveal key={area.name} delay={index * 0.06}>
              <article
                className={`group rounded-2xl border border-primary/10 bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${
                  index % 3 === 1 ? "lg:translate-y-3" : ""
                }`}
              >
                <div className="mb-5 inline-flex rounded-full bg-primary/10 p-3 text-primary transition-colors group-hover:bg-primary group-hover:text-white">
